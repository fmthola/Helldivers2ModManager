...
set -euo pipefail
cd "$(dirname "$0")/.."
PROJECT_KEY="helldivers2modmanager-linux"
...
# Coverage: generate an LCOV report and rewrite the absolute source paths to
# project-relative ones so the scanner (which mounts the project) can map them.
if cargo llvm-cov --version >/dev/null 2>&1; then
  echo ">> generating Rust coverage"
  cargo llvm-cov --manifest-path src-tauri/Cargo.toml --lib --lcov --output-path src-tauri/lcov.info
  sed -i "s#^SF:$PWD/#SF:#" src-tauri/lcov.info
else
